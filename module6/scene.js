 ...
 *************************************************************/
//Template from the code examples, Modifications made by 1101097 Ville Peltola
//
// Parameters
var width = 800,
    height = 600
...
var custParticleSystemFire = null;
var custParticleSystemSmoke = null;

var trees = [];
var s = null;
var clouds = null;
var cloudsObj = null;
cloudsObj = new THREE.Object3D();
var lime = THREE.ImageUtils.loadTexture("lime.png");
var pine = THREE.ImageUtils.loadTexture("pine.png");
...
$(function(){
	...
	//Clouds
	//////////////////////////////////////////////////
	clouds = THREE.ImageUtils.loadTexture("clouds.png");
	loader.load("meshes/sky.js", handlerForSky);
	function handlerForSky(geometry, materials) {
		s = new THREE.Mesh(geometry, new THREE.MeshBasicMaterial({
			map:clouds,
			transparent: true,
...
function createTree(texture,  x,  z)
{
	...
	var plane2 = new THREE.Mesh( new THREE.PlaneGeometry(10,10),
								 new THREE.MeshBasicMaterial({
								 map:texture,
								 side: THREE.DoubleSide,
								 depthWrite: true,
	...
	plane1.position.z = z;
	plane1.position.y = 4;
	plane1.rotation.x = 180 * Math.PI / 180;

	plane2.position.x = x;
	plane2.position.z = z;
	plane2.position.y = 4;
	plane2.rotation.x = 180 * Math.PI / 180;
	plane2.rotation.y = 90 * Math.PI / 180;;
	
	plane1.renderDepth = 300;
	plane2.renderDepth = 300;
...
createTree(lime, 5, 5);
createTree(pine, 20, 20);
createTree(lime, 25, 25);
createTree(lime, -5, -5);
createTree(lime, -20, -20);
createTree(lime, 22, -2);
createTree(pine, 12, 22);
createTree(lime, 22, -52);
createTree(lime, 23, -62);
createTree(lime, 52, 62);
createTree(pine, 82, -32);
createTree(lime, 42, 32);
createTree(lime, 102, -2);
		
//////////////////////////////////////////////////////////////////////////
//Partclesystems
//////////////////////////////////////////////////////////////////////////
	var fireText = THREE.ImageUtils.loadTexture("fire.png");
	var smokeText = THREE.ImageUtils.loadTexture("smoke.png")
    // Create our improved particle system object.
	
    custParticleSystem = new CustomParticleSystem( {
	maxParticles: 20,
	energyDecrement: 0.95,
	throughPutFactor: 20.5,
	//scale: 0.02,
	//movement: 0.03,
	...
	onParticleInit: function(particle){
		...
		var vy = (Math.random()*(0.9-0.5)+0.5);
		
		particle.scale = 20;
		var x = (Math.random()*(1.5-1.0)+1.0);

		
		var y = (Math.random()*(0.5-1.0)+0.5);
	
		var z = (Math.random()*(1.5-1.0)+1.0);
		
		
	   particle.set(x,y,z); 
	   particle.velocity = new THREE.Vector3(0,vy,0);
	    // particle life force
	    particle.energy = 0.50;
	...
	custParticleSystemSmoke = new CustomParticleSystem( {
	maxParticles: 10,
	energyDecrement: 0.5,
	throughPutFactor: 2.5,
	//scale: 0.02,
	//movement: 0.03,
	...
	onParticleInit: function(particle){
		...
		var y = (Math.random()*(1.5-1.0)+1.0);
	
		var z = (Math.random()*(1.5-1.0)+1.0);
		
		
	   particle.set(3.0,10.0,3.0); 
	   particle.velocity = new THREE.Vector3(0,vy,0);
	    // particle life force
	    particle.energy = 1.50;
		//particle.material.opacity = 0.0;
	},
	onParticleUpdate: function(particle,delta){
	    // Add velocity per passed time in seconds
		...
		//particle.opacity -= 0.1;
	    particle.energy -= (custParticleSystemSmoke.options.energyDecrement * delta);
	}
    });
	scene.add(custParticleSystemSmoke.ps);
